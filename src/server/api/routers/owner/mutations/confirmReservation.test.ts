...
const call = createCallerFactory(createTRPCRouter({ confirmReservation }));

describe("owner.confirmReservation", () => {
	it("rejects an unauthenticated caller", async () => {
		const { ctx } = makeCtx({ session: null });
		await expect(
			call(ctx).confirmReservation({ reservationId: ID }),
	...
	it("rejects confirming a non-pending reservation", async () => {
		const { ctx } = makeCtx({
			row: reservationRow({ status: "confirmed" }),
		});
		await expect(
			call(ctx).confirmReservation({ reservationId: ID }),
	...
	it("persists pending → confirmed for the owning restaurant", async () => {
		const updated = { id: ID, status: "confirmed" };
		...
		expect(patch.validatedAt).toBeInstanceOf(Date);
	});
});
